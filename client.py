...
class Client:
    ...
    def get_public_key():
        ...
        key_response = requests.get("https://localhost:5000/get_public_key", verify=False)
    ...
    def login(username, password):
        ...
        try:
            response = requests.post(f"{server_url}/login", json=payload, verify=False)
    ...
    def register(name, username, password, email):
        ...
        try:
            response = requests.post(f"{server_url}/register", json=payload, verify=False)
    ...
    def send_content(file_path, name, user_id):
        ...
        try:
            ...
            response = requests.post(f"{server_url}/upload", data=payload, files={"file": content}, verify=False)
            if response.status_code == 201:
                return True
            else:
                raise Exception("An error occurred: " + response.json()["status"])
    ...
    def refresh_content():
        ...
        try:
            response = requests.get(f"{server_url}/content", verify=False)
    ...
    def download_content(content):
        ...
        try:
            response = requests.get(f"{server_url}/download/{content_id}", stream=True, verify=False)
            if response.status_code == 200:
